"""
core/gui_console.py — Console UI và FlagInjector logic.
Tách khỏi main.py để dễ maintain, không đụng đến core_rs.
"""
...
from core import MemoryManager, PatternScanner, AttachTimeoutError

log = logging.getLogger(__name__)

# ──────────────────────────────────────────────
# Constants — FFlag map layout
# Sửa các giá trị này bằng Cheat Engine khi Roblox update struct.
# Xem README.md → "Cập nhật sau Roblox update" để biết chi tiết.
# ──────────────────────────────────────────────
...
class FlagInjector:
    """
    Locates và patches Roblox FFlags trong memory.
    ...
    def _attach(self) -> None:
        ...
        safe = self._mm.attach_raw(ROBLOX_EXE, ROBLOX_EXE)
        self._safe_handle    = safe
        self._process_handle = safe.value
        self._module_base    = safe.module_base
        self._module_size    = safe.module_size
    ...
    def _scan_offset(self) -> None:
        ...
        if offset is None:
            raise RuntimeError(
                ...
                "      Xem README.md → 'Cập nhật sau Roblox update' để biết cách fix."
            )
    ...
    def _read_entry_name(self, entry_data: bytes) -> Tuple[bytes, int]:
        ...
        if str_alloc > 0xF:
            ...
            return (bytes(name_bytes[:str_size]) if name_bytes else b""), str_size
    ...
    def _find_flag_addr(self, name: str) -> int:
        ...
        try:
            map_data = bytes(self._mem.read(self._process_handle, hashmap_addr, 56))
        except Exception:
            return 0
        ...
        try:
            bucket_data = bytes(self._mem.read(self._process_handle, bucket_base, 16))
        except Exception:
            return 0
