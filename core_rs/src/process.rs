...
// windows 0.62: HANDLE(pub *mut c_void) — không Send/Sync by default
// SAFETY: handle chỉ dùng trong single-threaded PyO3 context
...
impl SafeHandle {
    ...
    /// Lấy handle dưới dạng isize để truyền cho NtDll (NtRead/WriteVirtualMemory)
...
#[pymethods]
impl SafeHandle {
    /// Trả về handle value dưới dạng isize — dùng để truyền vào NtMemory
    #[getter]
    fn value(&self) -> isize {
        self.handle.0 as isize
    }
