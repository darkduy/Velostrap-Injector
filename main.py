...
class FlagInjector:
    ...
    def apply_json_results(self, json_path: str) -> List[Tuple[bool, str]]:
        """
        Parse json_path và apply mọi flag.
        Trả về list (ok, message) để gui_console hiển thị.
        Không print gì — toàn bộ output do caller quyết định.
        """
        ...
        return [self.apply_flag(key, val) for key, val in data.items()]

    def cleanup(self) -> None:
        """Release process handle và clear caches."""
...
def main() -> None:
    run_console(
        injector_factory=lambda: FlagInjector(),
